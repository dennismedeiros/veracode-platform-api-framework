...
/**
 * Supporting tool to build a byte array in memory
 * 
...
public class ByteArrayBuilder {
	...
	public ByteArrayBuilder append(byte[] value, final int offset, final int len) {
	...
	public ByteArrayBuilder append(char[] value, final int offset, final int len) {
		if (offset + len > value.length)
			throw new ArrayIndexOutOfBoundsException();
	...
	public ByteArrayBuilder append(Object value) {
		try {
		...
		// if the above did not work, instead append the result of the toString()
		// method.
	...
	public ByteArrayBuilder append(StringBuffer value) {
		byte[] b = value.toString().getBytes(Charset.defaultCharset());
		testAddition(b.length + 4);
		return this.append(b.length).append(b);
	}

	public ByteArrayBuilder append(ByteArrayBuilder value) {
	...
	public ByteArrayBuilder appendSpecial(long value, int length, boolean preserveNegative) {
		testAddition(length);
		{
